...
with st.sidebar:
  ...
  tipo = st.radio("Selecione o Cálculo", options=["Probabilidade Exata", "Menos que", "Mais que"])
...
if processar:
  ...
  if tipo == "Probabilidade Exata":
    ...
    tit = "Probablidades de Ocorrência"
  elif tipo == "Menos que":
    probs = poisson.cdf(x_vals, lamb)
    tit = "Probablidades de Ocorrência Igual ou Menor que"
  else:
    probs = poisson.sf(x_vals, lamb)
    tit = "Probablidades de Ocorrência Maior que"
