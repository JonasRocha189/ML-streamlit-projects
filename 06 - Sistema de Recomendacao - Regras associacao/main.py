...
if processar and uploaded_file is not None:
  try:
    ...
    regras = association_rules(frequent_itemsets, metric='confidence', min_threshold=confianca_minima, num_itemsets=2)
    ...
    if not regras_filtradas.empty:
      ...
      with col3:
        ...
        fig,ax = plt.subplots()
        scatter = ax.scatter(regras_filtradas['support'], regras_filtradas['confidence'], alpha=0.5,c=regras_filtradas['lift'], cmap='viridis')
